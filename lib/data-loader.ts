...
function normalizeToDetail(name: string, data: any): RepoDetail {
  ...
  return {
    ...
    machineSpec: data.__original?.machine_spec,
    documentReadingSummary: data.__original?.document_reading_summary,
...
function normalizeStatus(status: any): ResultStatus {
  ...
  if (s === 'not_run' || s === 'skipped' || s === 'not_executed' || s === 'not_configured' || s === 'not_attempted' || s === 'not_applicable') return 'not_run'
  return 'unknown'
}
